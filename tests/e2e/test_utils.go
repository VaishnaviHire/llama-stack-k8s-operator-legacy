...
func validateCRD(c client.Client, ctx context.Context, crdName string) error {
	...
	err := wait.PollWithContext(ctx, generalRetryInterval, ResourceReadyTimeout, func(ctx context.Context) (bool, error) {
...
func EnsureResourceReady(
	...
	return wait.Poll(pollInterval, timeout, func() (bool, error) {
		...
		err := c.Get(context.TODO(), types.NamespacedName{Name: name, Namespace: namespace}, obj)
		if err != nil {
			if errors.IsNotFound(err) {
				return false, nil
			}
...
func EnsureResourceDeleted(t *testing.T, c client.Client, gvk schema.GroupVersionKind, name, namespace string, timeout time.Duration) error {
	t.Helper()
	return wait.Poll(pollInterval, timeout, func() (bool, error) {
		obj := &unstructured.Unstructured{}
		obj.SetGroupVersionKind(gvk)
